...
if svgs_to_remove in js:
    js = js.replace(svgs_to_remove, new_muscles)
else:
    print("Could not find SVG defs to remove")

# 2. Revert rendering for main selector
old_selector_svg = "React.createElement(mg.icon,{size:20,color:mg.color})"
new_selector_img = "React.createElement('img',{src:mg.icon,className:'muscle-btn-icon',alt:mg.label})"
js = js.replace(old_selector_svg, new_selector_img)

# 3. Revert inline rendering
old_inline_svg = "React.createElement(mg.icon,{size:14,color:mg.color})"
new_inline_img = "React.createElement('img',{src:mg.icon,className:'inline-muscle-icon'})"
js = js.replace(old_inline_svg, new_inline_img)

with open('/Users/mfolwh/Desktop/TRA/app.js', 'w') as f:
    f.write(js)

print("JS restored to PNGs")

# 4. Modify CSS to remove black background using mix-blend-mode
with open('/Users/mfolwh/Desktop/TRA/styles.css', 'r') as f:
    css = f.read()

# Add mix-blend-mode: screen to .muscle-btn-icon
css = css.replace(".muscle-btn-icon{width:36px;height:36px;object-fit:cover;transform:scale(1.25)}", 
                  ".muscle-btn-icon{width:36px;height:36px;object-fit:cover;transform:scale(1.25); mix-blend-mode: screen;}")

# Add mix-blend-mode: screen to .inline-muscle-icon
css = css.replace(".inline-muscle-icon{width:24px;height:24px;object-fit:cover;border-radius:4px;overflow:hidden}",
                  ".inline-muscle-icon{width:24px;height:24px;object-fit:cover;border-radius:4px;overflow:hidden; mix-blend-mode: screen;}")

with open('/Users/mfolwh/Desktop/TRA/styles.css', 'w') as f:
    f.write(css)

print("CSS updated with mix-blend-mode")
