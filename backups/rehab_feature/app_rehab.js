...
function App(){
  ...
  const [settings,setSettings]=useState(()=>load(SETTINGS_KEY,{userWeight:'',muscles:[], rehab: {active: false, startDate: null}}));
        ...
        React.createElement('div',{className:'day-panel-body'},
          ...
          draft.exercises.map((ex,ei)=>React.createElement('div',{key:ei,className:'exercise-block'},
            React.createElement('div',{className:'ex-name-row'},
              settings.rehab?.active ? React.createElement('select', {
            ...
            settings.rehab?.active && React.createElement('div', {className: 'rehab-warning'}, '⚠️ Увага: працюй на 50% сили. Жодних потуг та затримок дихання!'),
            ...
            ex.sets.map((s,si)=>{
              ...
              return React.createElement('div',{key:si,className: si===0?'set-row set-row-first':'set-row'},
                ...
                React.createElement('input',{className:'set-input',type:(s.bw || settings.rehab?.active)?'text':'number',inputMode:settings.rehab?.active?'text':'decimal',placeholder:settings.rehab?.active?'хв/інт':(s.prevWeight||'кг'),value:s.bw?s.weight+' кг':s.weight,disabled:s.bw,onChange:e=>{setField(ei,si,'weight',e.target.value);setField(ei,si,'bw',false)}}),
...
Введіть нове значення (або залиште порожнім щоб видалити):`, d.weight % 1 === 0 ? d.weight : d.weight.toFixed(3));
  ...
  // ─── MAIN RENDER ───────────────────────────────────────────────
  return React.createElement('div',{id:'app-root', className: settings.rehab?.active ? 'rehab-mode' : ''},
